...
class EpisodeIndex:
    ...
    by_subj_run_class: Dict[Tuple[int, int, int], np.ndarray] # (sid, run, cls) -> local idxs
...
def build_episode_index(ds, run_size: int = 100, y_key: str = "Y", x_key: str = "X",
                        drop_last: bool = True) -> EpisodeIndex:
    f = ds.file
    ...
    subj_local_to_global = {sid: np.asarray(gidxs, dtype=np.int64) for sid, gidxs in buckets.items()}
    ...
    for sid, gidxs in subj_local_to_global.items():
        ...
        n_full = (n_used + (run_size - 1)) // run_size if not drop_last else (n // run_size)
        n_runs[sid] = n_full
...
def sample_support(sid: int, epi: EpisodeIndex, K_per_class: int, rng: random.Random):
    query_run = rng.choice(epi.runs_for_subj[sid])  # pick query run
    sup_runs = [r for r in epi.runs_for_subj[sid] if r != query_run]
    sup = []
    for c in epi.classes_for_subj[sid]:
        # pool = all trials of class c from runs != rq
        pool = []
        for run in sup_runs:
        ...
        sup.extend(take)
    return sup, query_run

def sample_query(sid: int, query_run: int, epi: EpisodeIndex, Q_per_class: int, rng: random.Random):
    if Q_per_class is None:
        return list(epi.by_subj_run[(sid, query_run)])
    que = []
    for c in epi.classes_for_subj[sid]:
        block = epi.by_subj_run_class.get((sid, query_run, c), [])
        take = _contiguous(block, min(Q_per_class, len(block)), rng)
        que.extend(take)
    return que
...
def fetch_by_indices(ds: KUTrialDataset, epi: EpisodeIndex, sid: int, local_idxs: List[int], device: torch.device, non_blocking: bool = True):
    ...
    for g in gidxs:
        ...
        assert sid_ == sid, f"Subject mismatch: got {sid_}, expected {sid} (global idx {g})"
        Xs.append(x)
        ys.append(y)
