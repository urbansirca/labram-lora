...
def load_runs(subj_dir: Path):
    ...
    for f in subj_dir.glob("*.gdf"):
        m = pat.match(f.name)
        if m:
            run_files.append((int(m.group(1)), f))
...
def preprocess(
    ...
    if apply_ica:
        try:
            ...
            if len(eog_chs) == 0:
                print("  ⚠️  No EOG channels found, skipping ICA artifact removal")
            else:
                print(f"  Found EOG channels: {eog_chs}")
                
                # Fit ICA on EEG channels only
                ica = ICA(n_components=None, random_state=42, max_iter=500, verbose=False)
                
                # Downsample for ICA fitting to speed up and reduce memory
                raw_for_ica = raw.copy().resample(100, verbose=False)  # 100 Hz is enough for ICA
                ica.fit(raw_for_ica, picks='eeg')
                
                # Find and exclude EOG components - use channels that actually exist
                try:
                    eog_indices, eog_scores = ica.find_bads_eog(raw, ch_name=eog_chs, verbose=False)
                    ica.exclude = eog_indices
                    print(f"  ✓ Removed {len(eog_indices)} EOG components: {eog_indices}")
                except Exception as e:
                    print(f"  ⚠️  Could not detect EOG components: {e}")
                    # Continue without EOG removal
                
                # Apply ICA to original (non-downsampled) data
...
def epochs_from_events(
    raw: mne.io.BaseRaw, events: np.ndarray, tmin: float = 3.0, tmax: float = 8.0
    ...
    for i, s0 in enumerate(starts):
        ...
        mask = (
            (events[:, 0] > s0)
            & (events[:, 0] < s1)
            & np.isin(events[:, 2], [CODES["CUE_LEFT"], CODES["CUE_RIGHT"]])
        )
        if not mask.any():
            continue
        cue = events[mask][0, 2]
...
def pick_27_with_aux(raw: mne.io.BaseRaw):
    """Keep EEG + EOG/EMG channels for artifact detection"""
    chs = {ch.upper(): ch for ch in raw.ch_names}

    # Select 27 EEG channels
    eeg_sel = [chs.get(ch, chs[ch.upper()]) for ch in EEG_27]

    # Select auxiliary channels
    aux_channels = ["EOG1", "EOG2", "EOG3", "EMGg", "EMGd"]
    aux_sel = []
    for ch in aux_channels:
        if ch in chs:
            aux_sel.append(chs[ch])
        elif ch.upper() in chs:
            aux_sel.append(chs[ch.upper()])

    raw_picked = raw.pick(eeg_sel + aux_sel, verbose=False)
    ...
    for ch in raw_picked.ch_names:
        ...
        elif "EMG" in ch.upper():
            channel_types[ch] = "emg"

    raw_picked.set_channel_types(channel_types)
    return raw_picked
...
def convert_subject(subj_dir: Path, z_score_norm: bool = True) -> tuple[np.ndarray, np.ndarray]:
    ...
    for run_file in load_runs(subj_dir):
        ...
        raw_eeg = pick_27_with_aux(raw_full.copy())
    ...
    events_cat = (
        np.vstack(all_events) if len(all_events) else np.empty((0, 3), dtype=int)
    )

    X, Y = epochs_from_events(
        raw_cat, events_cat, tmin=3.0, tmax=8.0
    )  # X: (T, 27, 2560)
    # Resample each 4 s epoch from 2048 -> 1000 samples (i.e., 200 Hz) to match LaBraM patch200 checkpoints
