...
from meta_helpers import build_episode_index, sample_support, sample_query, fetch_by_indices
...
class MetaEngine:
    ...
    def save_regular_checkpoint(self):
        if self.save_regular_checkpoints and self.metrics.epoch % self.save_regular_checkpoints_interval == 0:
            metric_val = self.metrics.val_accuracy if self.metrics.val_accuracy is not None else self.metrics.train_accuracy
    ...
    def meta_step(self, subjects_batch: List[int]):
        ...
        for sid in subjects_batch:
            ...
            Xs, ys = fetch_by_indices(self.train_ds, self.train_epi, sid, sup_idx, self.device)
            ...
            grads_q = torch.autograd.grad(Lq, fast, retain_graph=False, allow_unused=True)
    ...
    def meta_validate_epoch(self):
        ...
        for sid in self.S_val:
            ...
            Xs, ys = fetch_by_indices(self.val_ds, self.val_epi, sid, sup_idx, self.device)
            Xq, yq = fetch_by_indices(self.val_ds, self.val_epi, sid, que_idx, self.device)
...
if __name__ == "__main__":
    ...
    chans   = 62
    samples = 800          # (4 patches * 200) flattened to T
    classes = 2
    ...
    with torch.no_grad():
        _ = model(torch.zeros(1, chans, samples, device=device))

    # --------- optimizer / scheduler factories (explicit) ---------
    opt_factory = lambda m: torch.optim.Adam(m.parameters(), lr=1e-3, weight_decay=0.0)
    sch_factory = lambda opt: torch.optim.lr_scheduler.StepLR(opt, step_size=50, gamma=0.5)
    ...
    engine = MetaEngine(
        ...
        S_train=sm.S_train, S_val=sm.S_val, S_test=sm.S_test,
