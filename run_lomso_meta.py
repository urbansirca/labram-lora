...
def get_checkpoint_file(root_dir, model_name, leave_out_subjs, type="best"):
    ...
    # find the experiment dir matching the test subjects
            ...
            if model_name == "deepconvnet":
                if type == "best":
                    ...
                    if not final_ckpt_files:
                        raise ValueError(f"No final_checkpoint*.pt found in {experiment_dir}")
                ...
                if type == "best":
                    ...
                    final_ckpt_dirs = [
                        ...
                        if d.is_dir() and d.name.startswith("final_checkpoint")
                    ]
                    if not final_ckpt_dirs:
                        raise ValueError(f"No final_checkpoint* directories found in {experiment_dir}")
    ...
    if ckpt_file is None:
        raise ValueError(f"No matching experiment directory found for subjects {leave_out_subjs}")

    print(f"✅ Found checkpoint: {ckpt_file} for model {model_name}, leave_out {leave_out_subjs}, type {type}")
    return ckpt_file
...
def run_lomso(config_path: str):
    ...
    if not test_only:
        ...
        checkpoint_root = Path("lomso/alternating_run1") # I renamed it from lomso --> lomso_supervised to avoid confusion
    ...
    for model_name in models:
        ...
        for fold_idx, test_pair in enumerate(folds, start=1):
            ...
            all_results = tester.test_all_subjects(
                shots_list=shots_list, n_epochs=n_epochs, n_repeats=n_repeats, use_cv_epoch_selection=False
            )
