...
def run_lomso(config_path: str):
    ...
    if max_folds is not None:
        subjects = subjects[: max_folds]

    logger.info(f"Test subjects: {subjects}")
    ...
    if not test_only:
        lomso_root = Path("lomso/individual-subject")
    else:
        lomso_root = Path("test_only_lomso/final_results")
    ...
    for model_name in models:
        ...
        for fold_idx, subj in enumerate(subjects, start=1):
            if fold_idx < 40:
            ...
            cfg.setdefault("data", {})["leave_out"] = [subj]
            ...
            logger.info(
                f"Starting fold {fold_idx}/{len(subjects)} for model {model_name}: test={subj} experiment_name={experiment_name}"
            )
...
if __name__ == "__main__":
    run_lomso("hyperparameters/hyperparameters-individual.yaml") # set max_folds to limit number of folds for development purposes
